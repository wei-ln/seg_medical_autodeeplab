...
class Cell(nn.Module):
    ...
    def forward(self, prev_prev_input, prev_input):
        ...
        if self.downup_sample != 0:
            feature_size_h = self.scale_dimension(len(s1[0][0]), self.scale)
            feature_size_w = self.scale_dimension(len(s1[0][0][0]), self.scale)
...
class newModel(nn.Module):
    def __init__(self, network_arch, cell_arch,net_arch, num_classes, num_layers, filter_multiplier=20, lock_multiplier=5, step=5, cell=Cell,
        ...
        self.net_arch = torch.from_numpy(net_arch)
        ...
        initial_fm = args.filter_multiplier
        half_initial_fm = initial_fm // 2
        self.stem0 = nn.Sequential(
            nn.Conv2d(1, half_initial_fm, 3, stride=2, padding=1),
    ...
    def forward(self, x):
        ...
        two_last_inputs=[]
        two_last_inputs.append(stem0)
        two_last_inputs.append(stem1)
        skip_tuple = self.get_skip_index(self.net_arch)

        for i in range(self._num_layers):
            tmp = self.cells[i](two_last_inputs[-2], two_last_inputs[-1])
            new_output = tmp[-1]
            low_level_input = np.where(skip_tuple[:,1]==i)[0]
            if len(low_level_input) != 0:
                new_output += two_last_inputs[low_level_input[0]+2]
            two_last_inputs.append(new_output)
        last_output = two_last_inputs[-1]
        # else:
        return last_output

    def get_skip_index(self, level_list_tmp):
        level_list = np.zeros(len(level_list_tmp) + 1)
        level_list[1:] = level_list_tmp
        level_list = level_list.astype(int)

        skip_tuple = []
        down_list = [0, -1, -1, -1]
        before = level_list[0]
        for i in range(1, len(level_list)):
            if level_list[i] >= before:
                down_list[level_list[i]] = i
            else:
                skip_tuple.append([down_list[level_list[i]] - 1, i - 1])
                down_list[level_list[i]] = i
            before = level_list[i]
        return np.array(skip_tuple)
    def get_params(self):
        bn_params = []
        non_bn_params = []
