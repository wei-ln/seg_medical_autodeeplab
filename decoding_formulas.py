...
class Decoder(object):
    def __init__(self, alphas, betas, steps):
        ...
        self.network_space = torch.zeros(8, 4, 3)
    ...
    def viterbi_decode(self):
        ...
        for layer in range(self.network_space.shape[0]):
            if layer == 0:
                prob_space[layer][0] = self.network_space[layer][0][1] #在[4,1]这个点
                prob_space[layer][1] = self.network_space[layer][0][2] #在[8，1]这个点
                path_space[layer][0] = 0 #在[16，1]这个点，没有这个点
                path_space[layer][1] = -1 #在[32，1]这个点
            else:
                for sample in range(self.network_space.shape[1]): #4个分辨率层
                    if layer - sample < - 1:
                        continue
                    local_prob = []
                    for rate in range(self.network_space.shape[2]):  # k[0 : ➚, 1: ➙, 2 : ➘]
                        if (sample == 0 and rate == 2) or (sample == 3 and rate == 0):
                            continue #最上面一行➘操作和最下面一行的➚操作全部忽略
                        else:
                            local_prob.append(prob_space[layer - 1][sample + 1 - rate] *
                                              self.network_space[layer][sample + 1 - rate][rate])#计算opath概率 维特比
                    prob_space[layer][sample] = np.max(local_prob, axis=0)
                    rate = np.argmax(local_prob, axis=0)
                    path = 1 - rate if sample != 3 else -rate
                    path_space[layer][sample] = path  # path[1 : ➚, 0: ➙, -1 : ➘] 选择的是哪一个输出op

        output_sample = prob_space[-1, :].argmax(axis=-1)
        actual_path = np.zeros(8).astype('uint8')
