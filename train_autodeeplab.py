...
class Trainer(object):
    def __init__(self, args):
        ...
        if self.args.dataset =='2d':
            ...
            self.data_dict, self.nclass = make_data_loader_3d_patch(args, **kwargs)
        ...
        # Define network
        model = AutoDeeplab(self.nclass, 8, self.criterion, self.args.filter_multiplier,
        ...
        self.scheduler = LR_Scheduler(args.lr_scheduler, args.lr,
                                      args.epochs, self.data_dict['num_train'], min_lr=args.min_lr)
    ...
    def training(self, epoch):
        ...
        for i in range(num_img_tr):
            image, target = torch.FloatTensor(self.data_dict['train_data'][i]), torch.FloatTensor(self.data_dict['train_mask'][i])
            if self.args.cuda:
            ...
            if epoch % self.args.alpha_epoch == 0 :
                ...
                for j in range(num_img_valid):
                    image_search, target_search = torch.FloatTensor(self.data_dict['valid_data'][j]), torch.FloatTensor(self.data_dict['valid_mask'][j])
                    if self.args.cuda:
    ...
    def validation(self, epoch):
        ...
        for i in range(num_img_test):
            image, target = torch.FloatTensor(self.data_dict['test_data'][i]), torch.FloatTensor(self.data_dict['test_mask'][i])
            if self.args.cuda:
                image, target = image.cuda(), target.cuda()
            with torch.no_grad():
                output = self.model(image)
